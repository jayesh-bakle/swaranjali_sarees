...
export default function Navbar() {
  ...
  const linkClass = ({ isActive }) =>
    `px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive ? 'text-primary-600 bg-primary-50' : 'text-slate-700 hover:text-primary-600 hover:bg-slate-100'}`
    ...
    <header className="bg-white shadow-soft sticky top-0 z-50">
      ...
      <nav className="container-app flex items-center justify-between h-16">
        ...
        <div className="flex items-center gap-1">
          {/* Wishlist icon */}
          {user && !isAdmin && (
            <Link to="/wishlist" className="relative p-2 rounded-lg hover:bg-slate-100 transition-colors" aria-label="Wishlist">
          ...
          {user ? (
            <div className="relative">
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-slate-100 py-2 animate-fade-in">
