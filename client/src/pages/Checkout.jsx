...
export default function Checkout() {
  ...
  // Apply a coupon against the current subtotal (server-authoritative discount)
          ...
          <div className="bg-white rounded-xl shadow-soft p-6 sticky top-24">
              ...
              {appliedCoupon ? (
                <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                  <div className="text-sm">
                    ...
                    <span className="text-green-600 ml-2">-{formatINR(appliedCoupon.discount)}</span>
                  </div>
                  <button onClick={removeCoupon} className="text-xs text-green-700 font-medium hover:underline">
                    Remove
                  </button>
                </div>
              ) : (
            ...
            <div className="border-t border-slate-100 pt-4 mb-6">
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-bold text-primary-700">{formatINR(discountedTotal)}</span>
              </div>
            </div>

            <button onClick={handlePlaceOrder} disabled={placing || !selectedAddress} className="btn-primary w-full text-base py-3">
              {placing ? 'Processing...' : `Place Order • ${formatINR(discountedTotal)}`}
            </button>
